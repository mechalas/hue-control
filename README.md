...
Without any arguments, this command prints a complete listing of all scenes on the brigde, along with the light presets for each light in the scene. **This can take a very long time, as scene light states must be fetched individually. The more scenes you have on the bridge, the longer it takes. And because the Hue app creates a set of "default" scenes for each room, the more rooms you have, the more scenes you have.** 

For large Hue installs, it's generally better to use the **--summary** or **--no-light-states** options, in concert with the **--ignore-autogenerated** option to get a quick listing of the scenes that are most important, and then provide a scene ID or list of IDs to get detailed listings of the desired scenes.
